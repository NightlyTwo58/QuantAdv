...
def run_suite(model, loader, name, fp32_ref=None, eps=DEFAULT_EPS):
    ...
    safe_set(
        "clean_acc", lambda: sanity_check_accuracy(model, loader), "clean_acc failed"
    )
    safe_update(
        lambda: run_fgsm_pgd(model, loader, eps=eps),
        "FGSM/PGD failed",
        {
            "FGSM": lambda: results.get("FGSM", None),
            "PGD": lambda: results.get("PGD", None),
        },
    )
    safe_set(
        "AutoAttack",
        lambda: run_autoattack(model, loader, eps=eps),
        "AutoAttack failed",
    )
    safe_update(
    ...
    if fp32_ref is not None:
        safe_set(
            "Transfer_from_FP32",
            lambda: transfer_attack(fp32_ref, model, loader, eps=eps),
            "transfer_attack failed",
        )
        safe_set(
            "MIM_Transfer",
            lambda: transfer_attack_mim(fp32_ref, model, loader, eps=eps),
    ...
    safe_update(
        lambda: run_random_noise_seeded(model, loader, eps=eps),
    ...
    if count_quant_layers(model) > 0:
        safe_update(
            lambda: run_bpda(model, loader, eps=eps, n_restarts=BPDA_RESTARTS_SUITE),
            "BPDA failed",
            {"BPDA_PGD": None},
        )
        safe_update(
        ...
        safe_update(
            lambda: staircase_diagnostic(model, loader), "staircase_diagnostic failed"
        )
