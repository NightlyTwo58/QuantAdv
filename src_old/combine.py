"""
Combines the per-model CSVs/JSON written by the experiment runner into master
...
Run this after the experiment runner finishes all subprocesses:

    python src_old/combine_results.py
...
from .paths import (
    ...
    LAYERWISE_GLOB,
    TRAJECTORY_GLOB,
    RESULTS_COMBINED_CSV,
    SWEEP_COMBINED_CSV,
    ABLATION_COMBINED_CSV,
    LAYERWISE_COMBINED_CSV,
    TRAJECTORY_COMBINED_CSV,
    SCORECARD_CSV,
    ACCURACY_PLOT_PNG,
    SWEEP_PLOT_PNG,
    ABLATION_PLOT_PNG,
...
def plot_layerwise(df_layerwise):
    ...
    a proxy for network depth). Reveals whether the gradient dies at a src
...
def build_masking_scorecard(df_results):
    ...
    computed across the pipeline into a src sortable table + bar chart:
    ...
    This is the src table to put in a paper's appendix / to sort models by
    "how masked is this."
    """
    if df_results.empty:
        return pd.DataFrame()
